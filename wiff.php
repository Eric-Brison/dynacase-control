...
if( isset($_REQUEST['context']) && isset($_REQUEST['uploadModule']) ) {
  $contextName = $_REQUEST['context'];
  ...
  $ret = $context->uploadModule();
  if( $ret === false ) {
    $answer = new JSONAnswer(null, sprintf("Error uploadModule: %s", $context->errorMessage), true);
    echo $answer->encode();
    exit( 1 );
  }
...
if( isset($_REQUEST['context']) && isset($_REQUEST['getLocalModuleDependencies']) ) {
  $contextName = $_REQUEST['context'];
  $modulePathName = $_REQUEST['getLocalModuleDependencies'];
  ...
  $ret = $context->getLocalModuleDependencies($modulePathName);
  if( $ret === false ) {
    $answer = new JSONAnswer(null, sprintf("Error getLocalModuleDependencies('%s'): %s", $modulePathName, $context->errorMessage), true);
    echo $answer->encode();
    exit( 1 );
  }
